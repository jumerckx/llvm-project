//===- TranslateToCpp.cpp - Emit C++ matchers from pdl_constr -------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Emits readable C++ `RewritePattern` matchers from the `pdl_constr` dialect.
//
// `pdl_constr` already encodes the matcher tree as *structured* IR (linear
...
// emitting into. This mirrors the semantics that `PDLConstrToPDLInterp` lowers
// to a block CFG, but without flattening to blocks.
//
// Only the matcher is emitted. Each `pdl_constr.success @rewriters::@foo` lowers
...
/// Walks a `pdl_constr.matcher` region and emits a C++ `RewritePattern`.
...
private:
  ...
  /// Spelled C++ type for a pdl / pdl_constr type, unwrapping optionals.
...
LogicalResult MatcherCppEmitter::emitSuccess(SuccessOp op, StringRef failAction) {
  ...
  indent() << "// pdl_constr.success @" << leaf << " benefit("
           << op.getBenefit() << ")\n";
...
LogicalResult MatcherCppEmitter::emitSingle(Operation *op,
  ...
  return llvm::TypeSwitch<Operation *, LogicalResult>(op)
      ...
      .Default([&](Operation *o) {
        return o->emitError("unsupported pdl_constr op in C++ matcher emitter");
      });
}
...
LogicalResult MatcherCppEmitter::emitModule(Operation *root) {
  // Header / preamble.
  os << "//===- Generated by mlir-translate --pdl-constr-to-cpp. DO NOT EDIT. "
        "-===//\n";
  os << "//\n";
...
LogicalResult mlir::pdl_constr::translateToCpp(Operation *op, raw_ostream &os) {
