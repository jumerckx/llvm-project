...
private:
  ...
  LogicalResult collectPredicates(Block &block, MatcherInfo &info);
  ...
  Operation *getOrCreatePoolOp(Operation *modelOp,
                               ...
                               const DenseSet<Operation *> &alreadyUsed);
...
Operation *Combiner::getOrCreatePoolOp(
    Operation *modelOp, ArrayRef<Value> canonicalOperands,
    const DenseSet<Operation *> &alreadyUsed) {
  ...
  if (auto it = poolDedup.find(key); it != poolDedup.end()) {
    // A `foreach` is a *binding* occurrence, not merely a test: two loops over
    // the same range inside one matcher bind two independent elements (the
    // "value with both an `addi` user and a `muli` user" shape), so collapsing
    // them would silently rewrite the pattern. Across matchers the loops may
    // still be shared -- every alternative is simply tried per element -- so
    // only refuse to reuse a loop this matcher already exercises.
    if (!isa<ForEachOp>(modelOp) || !alreadyUsed.contains(it->second))
      return it->second;
  }
  ...
  // A `foreach` binds its loop variable as a body block argument rather than
  // as a result, so materialize an otherwise-empty body to carry the canonical
  // loop variable. The body stays empty on purpose: the pool records predicate
  // identity, not control flow -- the tree builder rederives the nesting.
  if (auto forEach = dyn_cast<ForEachOp>(poolOp)) {
    Block &block = forEach.getBody().emplaceBlock();
    block.addArgument(forEach.getElementType(), forEach.getLoc());
  }

  insertionIndex[poolOp] = nextInsertionIndex++;
  // A re-created `foreach` overwrites the key: a later occurrence dedups
  // against the innermost loop so far, which is sound and keeps sharing.
  poolDedup[key] = poolOp;
  return poolOp;
}

LogicalResult Combiner::collectPredicates(Block &block, MatcherInfo &info) {
  for (Operation &op : block) {
    ...
    // The pool is a flat predicate chain, so a `foreach` body is spliced into
    // that chain right after the loop itself and the tree builder nests
    // everything that follows inside the loop. Anything the input matcher
    // placed *after* the loop in the enclosing scope would silently move into
    // the body, so require the loop to be last.
    if (isa<ForEachOp>(&op) && op.getNextNode())
      return op.emitOpError(
          "combine-matchers requires `match.foreach` to be the last op in its "
          "block; ops following the loop would be absorbed into its body");

    // Operand canonicalization is just SSA value lookup: each input-matcher
    // operand is either the matcher root, a `foreach` loop variable, or the
    // result of a predicate we have already mapped, so its canonical
    // counterpart is in `toCanonical`.
    SmallVector<Value, 4> canonicalOperands;
    canonicalOperands.reserve(op.getNumOperands());
    for (Value operand : op.getOperands())
      canonicalOperands.push_back(info.toCanonical.lookup(operand));

    Operation *poolOp = getOrCreatePoolOp(&op, canonicalOperands, info.preds);
    ...
    // A `foreach` opens a nested scope: bind its loop variable to the pool
    // op's canonical one and keep collecting inside the body.
    if (auto forEach = dyn_cast<ForEachOp>(&op)) {
      info.toCanonical.map(forEach.getLoopVariable(),
                           cast<ForEachOp>(poolOp).getLoopVariable());
      if (failed(collectPredicates(forEach.getBody().front(), info)))
        return failure();
    }
  }
  return success();
...
LogicalResult Combiner::buildCanonicalPool() {
  ...
  for (MatcherOp matcher : inputMatchers) {
    ...
    if (failed(collectPredicates(body, info)))
      return failure();
...
/// SSA-native dependency: `a` must precede `b` iff `b` consumes any value
/// defined by `a`. No separate operand-ID / result-ID bookkeeping needed.
/// A `foreach` defines its loop variable as a body block argument rather than
/// a result, so block arguments count as definitions of their parent op. (The
/// canonical root is a block argument of the detached pool block, whose parent
/// op is null and therefore never matches.)
static bool dependsOn(Operation *a, Operation *b) {
  for (Value operand : b->getOperands()) {
    if (operand.getDefiningOp() == a)
      return true;
    if (auto arg = dyn_cast<BlockArgument>(operand))
      if (arg.getOwner()->getParentOp() == a)
        return true;
  }
  return false;
}
...
void Combiner::emitNode(OpBuilder &builder, Location loc, TreeNode *node,
  ...
  //
  // A `foreach` predicate is the exception: it already *is* a failure scope
  // (body failures advance to the next element, exhausting the range falls
  // through to the following sibling), so it never needs a `try` wrapper and
  // its success path is emitted into the loop body instead of after it.
  //
  ...
  while (node) {
    ...
    if (auto poolLoop = dyn_cast<ForEachOp>(node->pred)) {
      auto loop = ForEachOp::create(builder, loc,
                                    mapping.lookup(poolLoop.getRange()));
      {
        OpBuilder::InsertionGuard guard(builder);
        builder.setInsertionPointToStart(&loop.getBody().front());
        IRMapping bodyMapping = mapping;
        bodyMapping.map(poolLoop.getLoopVariable(), loop.getLoopVariable());
        emitNode(builder, loc, node->successPath.get(), bodyMapping);
      }
      node = node->failurePath.get();
      continue;
    }
...
// them).
//
// A navigation op may be sunk into a `try` / `switch_*` case / `foreach` body
// region only when *all* of its uses live inside that single region (otherwise
// it must stay in the enclosing region, which dominates every use). The
// range operand of a `foreach` is a use by the loop op itself, so the op
// producing it correctly stays outside the loop.
//===----------------------------------------------------------------------===//

namespace {

/// Pure navigation ops: side-effect-free value producers that can be freely
/// re-placed as long as they still dominate their uses. The `constant_*` ops
/// belong to the same category: they have no control effect, so they are sunk
/// to their first use and erased when dead (two identical literals from
/// different input matchers have already collapsed into one pool op, since the
/// pool key is `(op name, attributes, operands)` and they have no operands).
static bool isPureNavigationOp(Operation *op) {
  return isa<ConstantAttributeOp, ConstantTypeOp, ConstantTypesOp,
             GetOperandOp, GetOperandsOp, GetResultOp, GetResultsOp,
