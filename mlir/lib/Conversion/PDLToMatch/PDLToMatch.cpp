...
public:
  ...
  /// Emit a `match.matcher` for the wrapped `pdl.pattern`. On failure a
  /// diagnostic has been emitted and no matcher is left behind.
  LogicalResult emit();
...
private:
  ...
  LogicalResult emitNonTreePredicates();
  ...
  /// Return the match value for `pdlVal`, materializing a constant op for an
  /// unbound `pdl.attribute` / `pdl.type` / `pdl.types` literal. Returns null
  /// if the value is genuinely unbindable.
  Value getOrMaterialize(Value pdlVal);
...
Value MatchEmitter::getOrRegister(Value pdlVal, Value constrVal) {
  ...
  if (!it.second) {
    // Already visited — emit equality constraint. `pdl::ResultOp` /
    // `pdl::ResultsOp` are deliberately absent: `emitOperandConstraints`
    // already emits the equality that ties a result back to the operand it
    // feeds, so including them here would emit it twice.
    Value existing = it.first->second;
    if (isa_and_nonnull<pdl::ApplyNativeConstraintOp, pdl::ApplyNativeRewriteOp,
                        pdl::AttributeOp, pdl::OperandOp, pdl::OperandsOp,
...
Value MatchEmitter::getOrMaterialize(Value pdlVal) {
  if (Value mapped = valueMap.lookup(pdlVal))
    return mapped;

  // A literal is well-formed PDL but is never reached by navigation from the
  // root, so it is absent from `valueMap`. Materialize it as a constant, which
  // is what the reference `pdl -> pdl_interp` lowering does with
  // `pdl_interp.create_*`. A `pdl.attribute` with only a type
  // (`%a = attribute : %t`) or with neither (`%a = attribute`) is *not* a
  // literal — it has to be bound by navigation, so returning null is correct.
  Value materialized;
  if (auto attrOp = pdlVal.getDefiningOp<pdl::AttributeOp>()) {
    if (Attribute value = attrOp.getValueAttr())
      materialized = match::ConstantAttributeOp::create(builder, loc, value);
  } else if (auto typeOp = pdlVal.getDefiningOp<pdl::TypeOp>()) {
    if (TypeAttr value = typeOp.getConstantTypeAttr())
      materialized = match::ConstantTypeOp::create(builder, loc, value);
  } else if (auto typesOp = pdlVal.getDefiningOp<pdl::TypesOp>()) {
    if (ArrayAttr value = typesOp.getConstantTypesAttr())
      materialized = match::ConstantTypesOp::create(builder, loc, value);
  }

  // Cache the materialized value so a literal shared by two constraints is
  // materialized once (matching the reference, which emits one `create_*`).
  if (materialized)
    valueMap[pdlVal] = materialized;
  return materialized;
}
...
void MatchEmitter::emitOperationConstraints(
  ...
  for (auto [attrName, attr] :
    ...
    if (!existing) {
      // The attribute value is not necessarily a `pdl.attribute`: it may be
      // produced by a native constraint (e.g. `%attr = apply_native_constraint
      // "c"(...) : !pdl.attribute` used as `{"attr" = %attr}`). Such a value
      // carries no attribute-level constraints of its own; the equality
      // against the produced value is emitted by `emitNonTreePredicates` once
      // the constraint itself is lowered.
      if (auto attrOp = attr.getDefiningOp<pdl::AttributeOp>())
...
void MatchEmitter::emitUpwardTraversal(OpIndex opIndex, Value &pos,
  ...
  TypeSwitch<Operation *>(value.getDefiningOp())
      .Case([&](pdl::OperationOp operationOp) {
        ...
        // Get users and iterate. Everything emitted from here on belongs to
        // the loop body, so the insertion point moves inside and stays there:
        // the caller's `InsertionGuard` restores it once the whole matcher is
        // emitted, and a further upward traversal simply nests one level
        // deeper.
        Value usersVal = match::GetUsersOp::create(
            builder, loc,
            pdl::RangeType::get(builder.getType<pdl::OperationType>()),
            userPos);
        auto loop = match::ForEachOp::create(builder, loc, usersVal);
        builder.setInsertionPointToStart(&loop.getBody().front());
        Value opVal = loop.getLoopVariable();
...
LogicalResult MatchEmitter::emitNonTreePredicates() {
  for (Operation &op : pattern.getBodyRegion().getOps()) {
    LogicalResult emitted =
        TypeSwitch<Operation *, LogicalResult>(&op)
            .Case([&](pdl::ApplyNativeConstraintOp constraintOp)
                      -> LogicalResult {
              // Collect arguments, materializing any unbound literal. An
              // argument that is neither reachable from the root nor a
              // constant cannot be expressed in the match dialect; dropping
              // the constraint would silently widen the pattern, so fail.
              SmallVector<Value> args;
              args.reserve(constraintOp.getArgs().size());
              for (Value arg : constraintOp.getArgs()) {
                Value mapped = getOrMaterialize(arg);
                if (!mapped)
                  return constraintOp.emitError()
                         << "constraint argument is not reachable from the "
                            "pattern root and is not a constant; cannot lower "
                            "to the match dialect";
                args.push_back(mapped);
              }

              SmallVector<Type> resultTypes(
                  constraintOp.getResultTypes().begin(),
              ...
              for (auto [i, result] :
                   llvm::enumerate(constraintOp.getResults())) {
                Value constrResult = nativeConstr.getConstraintResults()[i];
                (void)getOrRegister(result, constrResult);
              }
              return success();
            })
            .Case([&](pdl::ApplyNativeRewriteOp rewriteOp) -> LogicalResult {
              // A native rewrite used during matching is a value producer (it
              // cannot fail). `pdl.apply_native_rewrite` is
              // `HasParent<"pdl::RewriteOp">`, so this case is unreachable for
              // valid PDL; `match.apply_native_rewrite` deliberately is not
              // confined that way, so keep the lowering symmetrical with the
              // constraint case above rather than leaving a hole here.
              SmallVector<Value> args;
              args.reserve(rewriteOp.getArgs().size());
              for (Value arg : rewriteOp.getArgs()) {
                Value mapped = getOrMaterialize(arg);
                if (!mapped)
                  return rewriteOp.emitError()
                         << "rewrite argument is not reachable from the "
                            "pattern root and is not a constant; cannot lower "
                            "to the match dialect";
                args.push_back(mapped);
              }
            ...
            .Case([&](pdl::ResultOp resultOp) -> LogicalResult {
            ...
            .Case([&](pdl::ResultsOp resultOp) -> LogicalResult {
              if (valueMap.count(resultOp))
                return success();
              Value parentVal = valueMap.lookup(resultOp.getParent());
              if (!parentVal)
...
LogicalResult MatchEmitter::emit() {
  ...
  // On failure, drop the half-built matcher: the pattern is left in place so
  // the diagnostic points at IR that is still there.
  if (failed(emitNonTreePredicates())) {
    matcher.erase();
    return failure();
  }
...
void PDLToMatchPass::runOnOperation() {
  ...
  for (pdl::PatternOp pattern :
    ...
    MatchEmitter emitter(builder, pattern, rewriterModule,
                             rewriterSymbolTable);
    // A pattern that cannot be expressed in the match dialect is left in the
    // module (a diagnostic has already been emitted); erasing it would drop
