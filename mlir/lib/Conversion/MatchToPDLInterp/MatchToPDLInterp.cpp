...
//   * `match.foreach { ... }` lowers to `pdl_interp.foreach`. Its body is
//     lowered with `failureBlock` set to a `pdl_interp.continue` block (next
//     iteration); when the loop exhausts, control transfers to a fresh
//     "after-loop" block into which subsequent siblings are lowered. This is
//     the same shape as `match.try`, except for where body failures go. As
//     with `switch_*`, when the loop is the last op in its region the
//     after-loop target collapses to the enclosing `failureBlock`.
...
private:
  ...
  /// body region while we are lowering a `match.foreach` body (mirroring the
  ...
  LogicalResult lowerForEach(ForEachOp op);
...
LogicalResult Lowerer::lowerOp(Operation *op) {
  return TypeSwitch<Operation *, LogicalResult>(op)
      ...
      .Case<ForEachOp>([&](auto o) { return lowerForEach(o); })
...
LogicalResult Lowerer::lowerForEach(ForEachOp op) {
  // Same shape as `lowerTry`: open a scope, lower the body with a new failure
  // destination, then continue with the siblings in the after-block. Like the
  // `switch_*` fall-through, the after-block collapses to the enclosing
  // failure scope when the loop is the last op in its region.
  Block *afterLoop = op->getNextNode() ? newBlock() : failureBlock;

  builder.setInsertionPointToEnd(currentBlock);
  auto foreach = pdl_interp::ForEachOp::create(
      builder, op.getLoc(), lookup(op.getRange()), afterLoop,
      /*initLoop=*/true);
  map(op.getLoopVariable(), foreach.getLoopVariable());

  // Append a continue block at the end of the foreach region. Body failures
  // and fall-off-end of the body branch here, i.e. to the next iteration.
  ...
  // there were none, this scope is terminated (the loop's successor is
  // already the enclosing failure block).
  currentBlock = op->getNextNode() ? afterLoop : nullptr;
  return success();
}
