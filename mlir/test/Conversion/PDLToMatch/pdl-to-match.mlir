...
// CHECK:         apply_native_constraint "constraint"(%{{.*}} : !pdl.operation) is_negated = true
...
// value with `get_users` + `match.foreach`, then tied back with `equal`. The
// matcher takes the name of the pattern and forwards both roots. Everything
// that follows the traversal is emitted inside the loop body.
//===----------------------------------------------------------------------===//
...
// `get_users` nor `foreach` and forwards one operation.
...
// gets its own upward traversal. The two traversals are two *nested* loops: the
// second `get_users` is emitted inside the first loop's body, so a pattern
// asking for two distinct users cannot collapse into one loop.
//===----------------------------------------------------------------------===//
